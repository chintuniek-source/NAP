...
export default function ImageManagement({ onOpenJoin }) {
  ...
  return (
    <section id="image-management" className="py-24 bg-[#FFF9F2] border-t-3 border-[#1E1E24] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            ...
            <p className="mt-3 text-lg font-bold text-[#FF6584] font-handwriting text-2xl">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <button
              ...
              className="px-6 py-3 rounded-2xl font-black text-sm bg-[#1E1E24] text-white border-2 border-[#1E1E24] shadow-[4px_4px_0px_#FF8A5B] hover:shadow-[2px_2px_0px_#FF8A5B] hover:translate-x-0.5 hover:translate-y-0.5 transition-all flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {services.map((srv) => (
            <div
              key={srv.id}
              className={`${srv.bgCard} p-8 sm:p-10 rounded-3xl border-3 border-[#1E1E24] shadow-[7px_7px_0px_#1E1E24] flex flex-col justify-between hover:shadow-[10px_10px_0px_#1E1E24] hover:-translate-y-1 transition-all`}
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="w-14 h-14 rounded-2xl bg-white border-2 border-black shadow-[2px_2px_0px_#1E1E24] flex items-center justify-center text-3xl">
                    {srv.icon}
                  </div>
                  <span className="text-[11px] font-black px-3 py-1 rounded-full bg-white text-[#1E1E24] border border-black shadow-[1px_1px_0px_#1E1E24] uppercase tracking-wider">
                    {srv.tag}
                  </span>
                </div>
                ...
                {/* Deliverables */}
                <div className="mt-6 pt-5 border-t-2 border-[#1E1E24]/15 space-y-2.5">
                  <p className="text-xs font-black uppercase tracking-wider text-gray-500">Core Deliverables</p>
                  {srv.deliverables.map((item, idx) => (
                    <div key={idx} className="flex items-start gap-2.5">
                      <CheckCircle2 className="w-4 h-4 text-[#06D6A0] stroke-[3] mt-0.5 flex-shrink-0" />
              ...
              <div className="mt-8 pt-5 border-t-2 border-[#1E1E24]/15 flex items-center justify-between">
                <p className="text-xs italic font-semibold text-gray-600">
                  {srv.quote}
                </p>
                <button
                  onClick={() => { setSelectedService(srv.name); setShowInquiryModal(true); }}
                  className="px-4 py-2 rounded-xl text-xs font-black bg-white hover:bg-gray-100 text-[#1E1E24] border-2 border-black shadow-[2px_2px_0px_#1E1E24] flex items-center gap-1 flex-shrink-0"
                >
                  <span>Inquire</span>
        ...
        {/* Consulting Banner */}
        <div className="mt-14 bg-gradient-to-r from-[#FF8A5B] to-[#FF6584] p-8 sm:p-10 rounded-3xl border-3 border-[#1E1E24] shadow-[8px_8px_0px_#1E1E24] text-white flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <span className="bg-white text-black font-black text-xs px-3 py-1 rounded-full border border-black shadow-[2px_2px_0px_black] uppercase">
          ...
          <button
            ...
            className="px-7 py-4 rounded-2xl font-black text-base bg-[#FFD166] text-[#1E1E24] border-2 border-[#1E1E24] shadow-[4px_4px_0px_#1E1E24] hover:shadow-[2px_2px_0px_#1E1E24] hover:translate-x-0.5 hover:translate-y-0.5 transition-all flex-shrink-0"
          >
      ...
      {showInquiryModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border-3 border-[#1E1E24] shadow-[8px_8px_0px_#1E1E24] max-w-lg w-full p-6 sm:p-8 relative">
            <div className="flex items-center justify-between pb-4 border-b-2 border-gray-200">
              <div>
                <span className="text-xs font-black uppercase text-[#FF8A5B]">Image Management PR Wing</span>
                <h4 className="font-black text-2xl text-[#1E1E24] font-display">Project Brief Inquiry</h4>
              </div>
              <button 
            ...
            {!inquirySubmitted ? (
              <form onSubmit={handleInquirySubmit} className="mt-5 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    className="w-full px-4 py-2.5 rounded-xl border-2 border-[#1E1E24] bg-gray-100 text-sm font-bold"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-xl border-2 border-[#1E1E24] text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[#FF8A5B]"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-black uppercase tracking-wider text-gray-600 mb-1">
                ...
                <button
                  type="submit"
                  className="w-full py-3.5 rounded-xl font-black text-base bg-[#06D6A0] hover:bg-[#00c28e] text-[#1E1E24] border-2 border-[#1E1E24] shadow-[4px_4px_0px_#1E1E24] flex items-center justify-center gap-2"
                >
              ...
              <div className="py-8 text-center space-y-3">
                <div className="w-16 h-16 rounded-full bg-[#06D6A0] text-black flex items-center justify-center mx-auto border-2 border-black shadow-[3px_3px_0px_#1E1E24]">
                  <CheckCircle2 className="w-8 h-8" />
                </div>
                <h5 className="text-2xl font-black font-display text-[#1E1E24]">Brief Received!</h5>
                <p className="text-sm font-semibold text-gray-700 max-w-sm mx-auto">
                  Our PR & Brand Counsellor team will review <strong>{formData.brandName || 'your brand'}</strong> and reply within 24 hours.
                </p>
                <button
                  onClick={() => setShowInquiryModal(false)}
                  className="mt-4 px-6 py-2.5 rounded-xl bg-[#FFD166] text-[#1E1E24] font-black text-xs border-2 border-black"
                >
                  Done
                </button>
              </div>

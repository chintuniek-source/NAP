...
export default function ImagePRPage({ onOpenJoin }) {
  ...
  const pillars = [
    ...
    },
    {
      ...
      color: 'bg-[#FFF3EC]',
      badgeColor: 'bg-[#FF8A5B] text-white',
    ...
    },
    {
      ...
      color: 'bg-[#FFFBEA]',
      badgeColor: 'bg-[#FFD166] text-black',
    ...
    },
    {
      ...
      color: 'bg-[#F6F0FD]',
      badgeColor: 'bg-[#845EC2] text-white',
  ...
  return (
    <div className="pt-24 pb-20 bg-[#FFF9F2]">
      
      {/* Banner */}
      <section className="bg-gradient-to-b from-[#FFE8D6] to-[#FFF9F2] py-16 px-4 sm:px-6 lg:px-8 border-b-3 border-[#1E1E24]">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-5xl sm:text-7xl md:text-8xl font-black text-[#1E1E24] font-display tracking-tight">
            Image PR
          </h1>
          
          <p className="mt-4 text-2xl sm:text-3xl font-extrabold text-[#FF6584] font-handwriting">
            ...
            {pillars.map((p) => (
              <button
                ...
                className={`badge-sticker transition-all ${
                  selectedPillar === p.id 
                    ? 'bg-[#1E1E24] text-white shadow-[3px_3px_0px_#FF8A5B]' 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 space-y-16">
        ...
        {pillars.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className={`${p.color} rounded-3xl border-3 border-[#1E1E24] shadow-[10px_10px_0px_#1E1E24] p-8 sm:p-12 animate-in fade-in zoom-in-95 duration-200`}
            >
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-6 border-b-2 border-[#1E1E24]/20">
                <div className="flex items-center gap-3">
                  <div className="w-16 h-16 rounded-2xl bg-white border-2 border-black shadow-[3px_3px_0px_black] flex items-center justify-center text-4xl">
                    {p.icon}
                  </div>
                  <div>
                    <h2 className="text-3xl sm:text-4xl font-black text-[#1E1E24] font-display">
                      {p.name}
                    </h2>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-8">
                <div className="lg:col-span-7">
                  ...
                  <div className="mt-6 space-y-3">
                    <p className="text-xs font-black uppercase tracking-wider text-gray-500">Key Deliverables</p>
                    {p.deliverables.map((d, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <div className="w-5 h-5 rounded-full bg-black text-white flex items-center justify-center text-xs flex-shrink-0 mt-0.5 font-bold">
                          ✓
                        </div>
                ...
                <div className="lg:col-span-5 flex flex-col justify-between bg-white p-6 sm:p-8 rounded-2xl border-2 border-black shadow-[4px_4px_0px_black]">
                  <div>
                    <div className="flex items-center gap-2 text-xs font-black uppercase text-[#FF8A5B]">
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-200">
                    <a
                      href="#inquiry"
                      className="w-full py-3 rounded-xl font-black text-xs bg-[#FF8A5B] text-white border-2 border-black shadow-[2px_2px_0px_black] flex items-center justify-center gap-2 hover:bg-[#ff7a45]"
                    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {pillars.map((p) => (
            <div
              key={p.id}
              onClick={() => setSelectedPillar(p.id)}
              className={`p-6 rounded-2xl border-2 border-[#1E1E24] cursor-pointer transition-all ${
                selectedPillar === p.id 
                  ? `${p.color} shadow-[5px_5px_0px_#1E1E24] -translate-y-1` 
                  : 'bg-white hover:bg-gray-50 shadow-[2px_2px_0px_#1E1E24]'
              }`}
            >
              <span className="text-3xl">{p.icon}</span>
              <h4 className="font-black text-lg text-[#1E1E24] font-display mt-2">{p.name}</h4>
              <p className="text-xs text-gray-600 font-semibold mt-1 line-clamp-2">{p.tagline}</p>
            </div>
          ))}
        </div>

        {/* Interactive Consultation / RFP Builder Form */}
        <section id="inquiry" className="bg-white rounded-3xl border-3 border-[#1E1E24] shadow-[8px_8px_0px_#1E1E24] p-8 sm:p-12 scroll-mt-28">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-8">
              <span className="px-3 py-1 rounded-full text-xs font-black bg-[#FF8A5B] text-white border border-black uppercase">
                Brand Consultation Form
              </span>
              <h3 className="text-3xl sm:text-4xl font-black text-[#1E1E24] font-display mt-3">
            ...
            {!inquirySubmitted ? (
              <form onSubmit={handleInquiry} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-xl border-2 border-[#1E1E24] text-sm font-bold bg-[#FFFDF9]"
                    />
                  </div>
                ...
                <button
                  type="submit"
                  className="w-full py-4 rounded-xl font-black text-base bg-[#FF8A5B] hover:bg-[#ff763e] text-white border-2 border-[#1E1E24] shadow-[4px_4px_0px_#1E1E24] flex items-center justify-center gap-2"
                >
              ...
              <div className="py-10 text-center space-y-3 bg-[#FFF3EC] rounded-2xl border-2 border-[#FF8A5B]">
                <div className="w-14 h-14 rounded-full bg-[#FF8A5B] text-white flex items-center justify-center mx-auto text-2xl border-2 border-black">
                  ✓
                </div>
                <h4 className="text-2xl font-black text-[#1E1E24] font-display">
